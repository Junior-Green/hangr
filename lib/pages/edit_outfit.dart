...
class _EditOutfitState extends State<EditOutfit> {
  ...
  @override
  void initState() {
    _nameEditingController = TextEditingController(text: widget._outfit.name);
    _primaryColorEditingController =
        TextEditingController(text: widget._outfit.primaryColor);
    _secondaryColorEditingController =
        TextEditingController(text: widget._outfit.secondaryColor);

    _isInfoValid = ValueNotifier<bool>(true);
    _isPrimaryColorValid = ValueNotifier<bool>(true);
    _isSecondaryColorValid = ValueNotifier<bool>(true);
    _isNameValid = ValueNotifier<bool>(true);
    _isEnabled = ValueNotifier<bool>(true);
    _isOutfitImageValid = ValueNotifier<bool>(true);
    _isPiecesValid = ValueNotifier<bool>(true);
    ...
    try {
      for (final id in widget._outfit.wearableIds) {
        final w = wearables.firstWhere((element) => element.id == id);
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => setState(
        () => _toggleGroup =
            ToggableImageGroup(_getTogglableImages(), _getInitialIndex()),
      ),
    );
    super.initState();
  }
  ...
  Future<bool?> _errorDialog() async => Alert(
        context: context,
        type: AlertType.none,
        title: 'Error',
  ...
  int _getInitialIndex() {
    final toggables = _getTogglableImages();

    for (int i = 0; i < toggables.length; i++) {
      if (toggables[i].value == widget._outfit.type) {
        return i;
      }
    }
    return -1;
